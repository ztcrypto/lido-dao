...
const Voting = getContract('LidoVoting')
...
async function proposeChangingVoteTime(voting, tokenManager, newVoteTimeSec, txOpts) {
  const calldata = await voting.contract.methods.changeVoteTime(newVoteTimeSec).encodeABI()
  const evmScript = encodeCallScript([{ to: voting.address, calldata }])
  const voteDesc = `Change voting period to ${newVoteTimeSec} seconds`
